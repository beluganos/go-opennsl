...
func newICMPFieldGroup(unit int) (opennsl.FieldGroup, error) {
	qset := opennsl.FieldQSet{}
	qset.Init()
	qset.Add(
		opennsl.FieldQualifyIpType,
		opennsl.FieldQualifyIpProtocol,
	)

	fg, err := opennsl.FieldGroupCreate(unit, &qset, opennsl.FIELD_GROUP_PRIO_ANY)
	if err != nil {
		log.Errorf("[ICMP] FieldGroupCreate error. %s", err)
		return 0, err
	}
...
func addICMPFieldEntry(unit int, group opennsl.FieldGroup, t opennsl.FieldIpType) error {

	entry, err := group.EntryCreate(unit)
	if err != nil {
		log.Errorf("[ICMP] EntryCreate error. %s", err)
		return err
	}

	entry.Qualify().IpType(unit, t)
	entry.Qualify().IpProtocol(unit, 58, 0xff)
	...
	if err := entry.Install(unit); err != nil {
		log.Errorf("[ICMP] Entry install error. %s", err)
		return err
	}
...
func addICMP(unit int) error {
	group, err := newICMPFieldGroup(unit)
	if err != nil {
		return err
	}

	if err := addICMPFieldEntry(unit, group, opennsl.FieldIpTypeIpv6); err != nil {
		return err
	}
...
func addIPFieldEntry(unit int, group opennsl.FieldGroup, dst string) error {
	...
	entry.Qualify().EtherType(unit, unix.ETH_P_IPV6, ETH_P_MASK)
	entry.Qualify().DstIp6(unit, ip, ipnet.Mask)
	entry.Action().AddP(unit, opennsl.NewFieldActionCosQCpuNew(ACTION_COS_QUE))
	entry.Action().AddP(unit, opennsl.NewFieldActionCopyToCpu())
...
func addIPAcl(unit int) error {
	...
	if err := addIPFieldEntry(unit, group, "2001:db8:100::1/64"); err != nil {
		return err
	}
...
func aclSetup(unit int, names []string) error {
	...
	for _, name := range names {
		...
		case "icmp":
			if err := addICMP(unit); err != nil {
				log.Errorf("addICMP error. %s", err)
				return err
			}
